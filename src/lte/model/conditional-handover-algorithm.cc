...
ConditionalHandoverAlgorithm::DoReportUeMeas(uint16_t rnti, LteRrcSap::MeasResults measResults)
{
    ...
    // CHO execution monitoring: if CHO command already delivered, check execution condition
    if (pendingIt != m_pendingCho.end() && pendingIt->second.m_choCommandDelivered)
    {
        ...
        {
            // Execution condition satisfied, trigger CHO
            EmitChoEventTrace(rnti,
        ...
        return; // Don't process as new CHO preparation
    }
...
ConditionalHandoverAlgorithm::ExecuteCho(uint16_t rnti)
{
    ...
    const Time reqTx = rarRx + m_randomAccessStepDelay;
    const Time reqRx = reqTx + m_randomAccessStepDelay;
    const Time setupTx = reqRx + m_randomAccessStepDelay;
    const Time setupRx = setupTx + m_randomAccessStepDelay;
    const Time completeTx = setupRx + m_randomAccessStepDelay;
...

    {
        EmitChoDetailedTraceAt(
            ...
            "actor=UE action=TX message=RA-Msg1(Preamble) to=Target-gNB(cellId=" +
        ...
        EmitChoDetailedTraceAt(
            ...
            "actor=Target-gNB(cellId=" + std::to_string(targetCellId) +
                ") action=RX message=RA-Msg1(Preamble)");
        ...
        EmitChoDetailedTraceAt(
            reqTx.GetMilliSeconds(),
            rnti,
            "Step5_RA_Msg3Req_TxByUe",
            "actor=UE action=TX message=RA-Msg3(Request) to=Target-gNB(cellId=" +
                std::to_string(targetCellId) + ")");
        EmitChoDetailedTraceAt(
            reqRx.GetMilliSeconds(),
            rnti,
            "Step5_RA_Msg3Req_RxByTarget",
            "actor=Target-gNB(cellId=" + std::to_string(targetCellId) +
                ") action=RX message=RA-Msg3(Request)");

        EmitChoDetailedTraceAt(
            setupTx.GetMilliSeconds(),
            rnti,
            "Step5_RA_Msg4ContRes_TxByTarget",
            "actor=Target-gNB(cellId=" + std::to_string(targetCellId) +
                ") action=TX message=RA-Msg4(ContentionResolution) to=UE");
        EmitChoDetailedTraceAt(
            setupRx.GetMilliSeconds(),
            rnti,
            "Step5_RA_Msg4ContRes_RxByUe",
            "actor=UE action=RX message=RA-Msg4(ContentionResolution) from=Target-gNB(cellId=" +
                std::to_string(targetCellId) + ")");
